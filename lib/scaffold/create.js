...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'counoscoincore-lib': '^' + bitcore.version,
    'counoscoincore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/counoscoincore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the counoscoincore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the counoscoincore-node directory and configuration
