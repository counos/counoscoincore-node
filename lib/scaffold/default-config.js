...
/**
 * Will return the path and default counoscoincore-node configuration. It will search for the
 * configuration file in the "~/..counoscoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.counoscoin');
  var defaultConfigFile = path.resolve(defaultPath, './counoscoincore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/counoscoind')
          }
        }
      }
    };
